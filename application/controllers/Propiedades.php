...
class Propiedades extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $vars['titulo']          = 'Propiedades';
        $vars['vista_contenido'] = 'view_propiedades';
        $this->load->view('template_white', $vars);
    }
    
    function show_propiedades(){
		$users_online=$this->inicio_model->show_propiedades_all();
    ...
    function show_propiedades_filtro(){
		$users_online=$this->inicio_model->show_propiedades_filtro();
				...
				<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 entrada">
					<div style="border:1px solid rgb(216, 221, 228);">
						...
						<div style="padding:20px;padding-bottom:0px;">
							...
							<div class="descripcionIntro">'.$user_on_line->ubicacion.'</div>
							<div class="row" style="margin:0px;">
								<a href="'.base_url().'propiedades/detalle_propiedad/'.$user_on_line->id.'"><button class="meinteresa col-lg-6 col-xs-6 col-sm-6">ME INTERESA</button></a>
								<a href="'.base_url().'propiedades/detalle_propiedad/'.$user_on_line->id.'"><button class="verPropiedad col-lg-6 col-xs-6 col-sm-6">VER PROPIEDAD&nbsp;&nbsp;<img src="'.base_url('site-img/property.svg').'" style="width:16px;position:relative;top:3px;"></button></a>
							</div>
						</div>
					</div>
    ...
    function show_propiedades_filtros(){
		$users_online=$this->inicio_model->show_propiedades_filtros();
	...
	public function detalle_propiedad() {
        ...
        $vars['vista_contenido'] = 'view_detalle_propiedad';
    ...
    public function get_propiedades(){
        ...
        $propiedades = $this->inicio_model->get_propiedades_images($id);
        $pro = array();
        foreach($propiedades as $p){
	        $imagedata = getimagesize(base_url('imagenes_propiedad/'.$id.'/').$p['nombre_imagen']);
            $pro[] = array('w' => $imagedata[0], 'h' => $imagedata[1], 'src' => base_url('imagenes_propiedad/'.$id.'/').$p['nombre_imagen']);
        }
    ...
    public function get_propiedad_info($id){
        $propiedad = $this->inicio_model->get_propiedades_info($id);
        return $propiedad;
    }
    
    //Búsqueda de desarrollos
    function busqueda_propiedades(){
		$users_online=$this->inicio_model->busqueda_propiedades();
		if($users_online->num_rows()>0){
			$url = base_url();
			$useronline_html='';
	...
	public function enquire(){
		...
		$email = $this->load->view('email_propiedad', $data, TRUE);
